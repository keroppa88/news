...
async function run() {
  ...
  const prompt = `
    ...
    4、「重要ニュース」を10個を厳選して　見出し（媒体名）yyyy/mm/dd を記述する。重要ニュースは複数媒体が報じるはずなので、報じた媒体を媒体名として、で区切って羅列する。3社以上は「等」と入れたうえで省略する。
    ...
    9、 例外なく全ての記事見出しには媒体名と年月日を付ける。（）で囲んだ中に媒体名を入れる。そのの後に年月日をyyyy/mm/ddの形式で付記すること。
