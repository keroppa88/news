...
async function run() {
  ...
  const prompt = `
    ...
    4. 「経済ニュース」10個を厳選【見出し】を並べてください。「重要ニュース」の内容と重複しない経済ニュースを選ぶこと。3社以上は「等」と入れたうえで省略する。
    5、「国内ニュース」5個を厳選して【見出し】を並べてください。3社以上は「等」と入れたうえで省略する。
    6、「海外ニュース」5個を厳選して【見出し】を並べてください。3社以上は「等」と入れたうえで省略する。
    7，「その他ニュース」としてエンタメ、スポーツ、社会などのニュースの中から厳選して2個から5個を並べてください。重要性の低いニュースは提示しない。
    各【見出し】の後ろに（）で囲んで媒体名を表記すること。
    8、（）で囲んだ中に媒体名を入れる。そのの後に年月日をyyyy/mm/ddの形式で付記すること。
    9、各媒体ごとに主要な記事を5つずつ提示していく。見出しは各媒体の見出しに基づくが、英語の場合は日本語に翻訳する。複数社が報じていないユニークな記事を必ず含ませる。
    （）で囲んだ中に媒体名を入れる。その後に年月日をyyyy/mm/ddの形式で付記すること。
    10、はい、承知いたしました。以下にニュースリストを分析し、厳選したニュースとまとめを提示します。という文言は不要。
    11、最後に総合的なまとめを200字以内の文章で表す。章分けや段落分け（改行）をせず、一つの連続した文章として出力。300文字以内で総合的な分析コメントを作成してください。最終チェック、全ての出力は日本語で行うこと。
    12、以下に表記例を示す。
