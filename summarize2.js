...
async function run() {
  ...
  const prompt = `
    ...
    2、最初に総合的なまとめを●コメント●として提示する。一つの連続した文章として250～300文字で総合的な分析コメントを作成する。
    ...
    5. 「経済ニュース」10個を厳選【見出し】を並べてください。「重要ニュース」の内容と重複しない経済ニュースを選ぶこと。3社以上は「等」と入れたうえで省略する。
    6、「国内ニュース」5個を厳選して【見出し】を並べてください。3社以上は「等」と入れたうえで省略する。
    7、「海外ニュース」5個を厳選して【見出し】を並べてください。3社以上は「等」と入れたうえで省略する。
    8，「その他ニュース」としてエンタメ、スポーツ、社会などのニュースの中から厳選して2個から5個を並べてください。重要性の低いニュースは提示しない。
    ...
    11、はい、承知いたしました。以下にニュースリストを分析し、厳選したニュースとまとめを提示します。という文言は不要。
    12、最終チェック、全ての出力は日本語で行うこと。
    12、以下に表記例を示す。
