...
async function run() {
  ...
  const prompt = `
    ...
    10、各媒体ごとに主要な記事を5つずつ提示していく。見出しは各媒体の見出しに基づくが、英語の場合は日本語に翻訳する。複数社が報じていないユニークな記事を必ず含ませる。
    （）で囲んだ中に媒体名を入れる。その後に年月日をyyyy/mm/ddの形式で付記すること。
    11、最終チェック1、「はい、承知いたしました。以下にニュースリストを分析し、厳選したニュースとまとめを提示します。」等という文言は不要。提示してはならない。
    12、最終チェック2、全ての出力は日本語で行うこと。出力から文字列「【】」は削除する。新たに「【】」を付け足す処理を行ってはならない、絶対ダメ。
    12、以下に表記例を示す。
