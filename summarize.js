...
async function run() {
  ...
  const prompt = `
    ...
    重要ニュースは複数媒体が報じるはずなので、報じた媒体を媒体名として、で区切って羅列する。3社以上は「等」で省略。
    ...
    8、各媒体ごとに主要な記事を5つずつ提示していく。見出しは各媒体の見出しに基づく。複数社が報じていないユニークな記事を必ず含ませる。
    ...
    ●重要ニュース●
    1. トランプ関税 米最高裁が違法判決（ロイター、ブルームバーグ、BBC）2026/02/20
    2. 露報道官「日露関係ゼロに低下」（ロイター、ブルームバーグ、BBC）2026/02/20
    3. 高市首相○○（ロイター、ブルームバーグ、BBC）2026/02/20
    4. 図書館刺傷 男を取り押さえた70代（ロイター、ブルームバーグ、BBC）2026/02/20
    5. 手術で体内に袋忘れる 痛みで発覚（yahoo、ブルームバーグ、BBC）2026/02/20
    6. 症状なくてもインフル? 注意点は（ロイター、ブルームバーグ、BBC）2026/02/20
    7. エヌビディア倒産説　声明（ロイター、ブルームバーグ、BBC）2026/02/20
    8. 演技後のしぐさ 中井亜美が解説（日経、ブルームバーグ、BBC）2026/02/20
