...
async function run() {
  ...
  const prompt = `
    ...
    ## ルール
    1. ニュース全体を俯瞰し、重要なキーワード・トピックを60〜100個抽出する。
    ...
    4. 媒体名（ロイター、BBC、ブルームバーグ、NYタイムズ、WSJ、BBC、日本経済新聞、Reuters、PR TIMES、ITmedia NEWS等）はキーワードに含めない。
    ...
    9. 低スコアのワードはなるべく短文もしくは単語で提示する。（ワードクラウド図に多くを収録提示するため）。
    10.最低でに60個以上のワードを必ず提示する。重要ワードの関連語彙なども可。
